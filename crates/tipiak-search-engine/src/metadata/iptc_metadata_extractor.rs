...
impl MetadataExtractor for IptcMetadataExtractor {
    ...
    fn extract(&self, path: &Path) -> Result<Option<MediaMetadata>, Box<dyn Error>> {
        ...
        for (tag, value) in iptc.get_all() {
            match tag {
                IPTCTag::ObjectName => {
                    extend_metadata_field!(metadata, title, sanitize_words(&value.join(" ")))
                }
                IPTCTag::Headline => {
                    extend_metadata_field!(metadata, title, sanitize_words(&value.join(" ")))
                }
                IPTCTag::Caption => {
                    extend_metadata_field!(metadata, description, sanitize_words(&value.join(" ")))
                }
                ...
                IPTCTag::Keywords => {
                    extend_metadata_field!(metadata, tags, sanitize_words(&value.join(" ")))
                }
                ...
                IPTCTag::ByLine => {
                    extend_metadata_field!(metadata, author, sanitize_words(&value.join(" ")))
                }
                IPTCTag::ByLineTitle => {
                    extend_metadata_field!(metadata, author, sanitize_words(&value.join(" ")))
                }
